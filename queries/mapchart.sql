...
localized AS (
    SELECT
        ...
        COALESCE(en_replacename.name, NULLIF(en_desc.text, ''), mapchart.description) AS Description_localized
    ...
    LEFT JOIN replacelist AS en_replacename
        ON en_replacename.category = "mapchart"
       AND en_replacename.id = mapchart.id
